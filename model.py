...
from shift_reduce_v1 import ShiftReduce
...
class MainModel(object):
    def __init__(self, n_words, action_dict, role_type_dict, dep_type_vocab=None,
                 pos_dict=None, pretrained_vec=None, l2_norm=0.001):
        ...
        # shift reduce parser
        self.shift_reduce = ShiftReduce(joint_config, self.encoder_output_dim, action_dict, role_type_dict)
    ...
    def get_word_embed(self, toks, pos_list=None, is_train=True):
    ...
    def input_embed(self, toks, chars, range, pos_list=None, dep_list=None, dep_head=None, is_train=True,
    ...
    def __call__(self, toks, chars, act_ids, acts, sent_range, roles=None, pos_list=None, dep_list=None, dep_head=None):
        
        context_emb = self.input_embed(toks, chars, sent_range, pos_list=pos_list, dep_list=dep_list, dep_head=dep_head,
                                       is_train=True, return_sent_vec=False)

        sent_vec = None
        log_prob_list, loss_roles, frames, aspect_terms, opinion_terms, pred_action_strs = \
            self.shift_reduce(toks, context_emb, sent_vec, oracle_actions=act_ids, oracle_action_strs=acts,
                              is_train=True, roles=roles)
        act_loss = -dy.esum(log_prob_list)
        role_loss = dy.esum(loss_roles) if loss_roles else 0
        loss = act_loss + 0.8 * role_loss + self.regularization_loss(self.l2_norm)
        
        return loss

    def decode(self, toks, chars, act_ids, acts, sent_range, roles=None,
               pos_list=None, dep_list=None, dep_head=None,  mtype='dev'):
        dy.renew_cg()

        context_emb = self.input_embed(toks, chars, sent_range, pos_list=pos_list, dep_list=dep_list, dep_head=dep_head,
                                       is_train=False, return_sent_vec=False, mtype=mtype)

        sent_vec = None
        losses, loss_roles, frames, aspect_terms, opinion_terms, pred_action_strs = \
                                            self.shift_reduce(toks,
                                                context_emb, sent_vec, oracle_actions=act_ids, oracle_action_strs=acts,
                                                              is_train=False)

        return 0, frames, aspect_terms, opinion_terms, pred_action_strs