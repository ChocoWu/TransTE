...
class StackLSTM(object):
    ...
    def iter(self):
        for (hx, cx), idx, history in zip(self.states, self.indices, self.history_info):
            yield hx, idx, history

    def push(self, input, idx, history=None):
        '''
        ...
        self.history_info.append(history)
    ...
    def pop(self):
        ...
        # history = self.history_info.pop()
        # return hx, idx, history
        return hx, idx

    def last_state(self):
        return self.states[-1][0], self.indices[-1], self.history_info[-1]
    ...
    def __getitem__(self, item):
        ...
        history = self.history_info[item]
